...
## Access model
All collaborators have **write access** to repositories. To contribute, clone the relevant repository directly and push branches to it — no need to fork. Only designated reviewers listed below have merge access to the main branches.
...
git pull origin dev
```
Resolve any conflicts locally and push the updated branch.

## 4. Open a pull request
- Target the **`dev`** branch of the repository. Do not raise pull requests against `prod`.
...
- Only the reviewers listed above have merge permissions; contributors should not attempt to merge their own PRs directly to `dev` or `prod`.
